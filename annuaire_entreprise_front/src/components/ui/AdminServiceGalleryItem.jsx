...
const AdminServiceGalleryItem = ({ service, openingPopUp, openingRestoring, isLast }) => {
    const [isEditing, setIsEditing] = useState(false);
    const [editName, setEditName] = useState(service.name);


    const handleSave = () => {
        updateService(service.id, editName);
        setIsEditing(false);
    };
    ...
    return (
        <>
            <div className="flex flex-col md:flex-row items-center hover:scale-101 transition-all  justify-between gap-4 md:gap-8 w-full h-auto py-2 px-4 ">
                ...
                {/* Boutons */}
                <div className="flex flex-row gap-2  justify-end items-center w-full md:items-end md:w-1/4">
                    {isEditing ? (
                        <button
                            onClick={handleSave}
...
export default AdminServiceGalleryItem;